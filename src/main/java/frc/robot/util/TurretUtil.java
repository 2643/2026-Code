...
public class TurretUtil {
        // private static double lastVirtualX = 0;
        // private static double lastVirtualY = 0;
        // private static final double VIRTUAL_DAMPING = 0.7; // 0-1, lower = more damping
        ...
        public static Pose2d getTargetPose(TargetType target) {
            switch (target) {
                case HUB:        return Constants.FieldConstants.getHubTarget().toPose2d();
                case LEFT_PASS:  return Constants.FieldConstants.getLeftPassTarget().toPose2d();
                case RIGHT_PASS: return Constants.FieldConstants.getRightPassTarget().toPose2d();
                default:         return Constants.FieldConstants.getHubTarget().toPose2d();
            }
        }
        ...
        public static TargetType getNearestPassTargetType(Pose2d robotPose) {
            ...
            double distLeft  = turret.getDistance(Constants.FieldConstants.getLeftPassTarget().toPose2d().getTranslation());
            double distRight = turret.getDistance(Constants.FieldConstants.getRightPassTarget().toPose2d().getTranslation());
        ...
        public static ShotSolution computeLeadShotSolution(Pose2d robotPose,
            ...
            // possible other filters --> robot speeds are bad and maybe need to be reduced when sent
            // robotVelX = robotVelX * 0.6;
            // robotVelY = robotVelY * 0.6;
    
            for (int i = 0; i < 5; i++) { // you could reduce loop speed to reduce oscillation
            ...
            // ai damping thingie test if this makes it better
            // also idk if you'll see it in constants but ai says to reduce the vel and accel of swivel
            // virtualX = lastVirtualX * VIRTUAL_DAMPING + virtualX * (1 - VIRTUAL_DAMPING);
            // virtualY = lastVirtualY * VIRTUAL_DAMPING + virtualY * (1 - VIRTUAL_DAMPING);
